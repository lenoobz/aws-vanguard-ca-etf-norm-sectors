...
import (
	...
	"github.com/lenoobz/aws-vanguard-ca-etf-norm-sectors/consts"
	"github.com/lenoobz/aws-vanguard-ca-etf-norm-sectors/entities"
	"github.com/lenoobz/aws-vanguard-ca-etf-norm-sectors/infrastructure/repositories/mongodb/models"
...
func (r *BreakdownMongo) UpdateSectorsBreakdown(ctx context.Context, funds []*entities.FundBreakdown) error {
	...
	for _, fund := range funds {
		...
		if _, err := col.UpdateOne(ctx, filter, update, opts); err != nil {
			r.log.Error(ctx, "update one failed", "error", err, "ticker", fund.Ticker)
			return err
		}
	}
